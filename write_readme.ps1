# write_readme.ps1
# Run this from D:\projects\agentic-research-analyst
# Usage: .\write_readme.ps1
...
Query
  v
Planner (70B)          -> 4-7 source-tagged sub-questions
  v
Orchestrator           -> asyncio.gather fan-out
  +- Web (Tavily)
  +- arXiv (arxiv pkg)
  +- GitHub (PyGithub + keyword extraction)
  v
Synthesizer (8B)       -> Findings: claim + verbatim quote + URL + confidence
  v
Writer (70B -> 8B)     -> Markdown report with inline [N] citations
  v
Verifier (70B -> 8B)   -> Per-claim verdicts + grounding score
  v
Final report
``````
...
# Write as UTF-8 without BOM (matches what git expects on Windows)
...
Write-Host ""
Write-Host "First 10 lines (sanity check):" -ForegroundColor Yellow
